    ...
    async def process_message(self, id_wpp: str, text: str) -> ChatResponse:
        try:
            ...
            if clean_text.lower() == "/resetar":
                user.state = "NEW"
                self.db.commit()
                return ChatResponse(text="Conversa reiniciada!\nEnvie uma mensagem para começar.")
            ...
            if clean_text.lower() in comandos_menu:
                ...
                if len(nome) < 2:
                    response = ChatResponse(text="Digite um nome válido.")
                elif len(nome) > 30:
                    response = ChatResponse(text="Nome muito longo. Digite apenas seu primeiro nome.")
                elif any(char.isdigit() for char in nome):
                    response = ChatResponse(text="O nome não deve conter números.")
